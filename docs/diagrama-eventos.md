...
| `SolicitudCreada`          | `CrearSolicitud`                                                                                                                                  | `Notificar` → "solicitud recibida" al productor · `emparejar-automatico` → asigna si hay vehículo, pago confirmado y el interruptor está activo |
| `PagoAprobado`             | `PagarConPasarela` (tarjeta OK) / `RevisarPago` (admin aprueba depósito)                                                                          | `Notificar` → productor ("pago confirmado") · `emparejar-automatico` → reintenta la asignación                                                  |
| `PagoRechazado`            | `PagarConPasarela` (tarjeta rechazada) / `RevisarPago` (admin rechaza)                                                                            | `Notificar` → productor ("vuelve a intentar")                                                                                                   |
| `PagoEnRevision`           | `RegistrarDeposito`                                                                                                                               | `Notificar` → correo al admin ("comprobante por revisar")                                                                                       |
...
Además del `Notificar` (correo), el subscriber **`notificaciones-app`** procesa casi todos
los eventos de cara al usuario y crea el aviso in-app (`NOTIF#<userId>`, campanita). El correo
quedó recortado a lo esencial (credenciales, pagos rechazados, revisión de depósito, retrasos,
incidencias, cancelación, stock); "solicitud recibida", "pago confirmado", "asignación",
"avance de estado" y "entrega" ya **no** generan correo, solo aviso in-app.
